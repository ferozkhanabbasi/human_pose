...
class TfPoseEstimator:
    ...
    def draw_humans(npimg, humans, imgcopy=False):
        import csv
        wimg = np.zeros([100,100,3],dtype=np.uint8)
        wimg.fill(255) # or img[:] = 255
        if imgcopy:
            npimg = np.copy(npimg)
        image_h, image_w = npimg.shape[:2]
        dim = (image_w, image_h)
        wimg = cv2.resize(wimg, dim, interpolation = cv2.INTER_AREA)
        centers = {}
        ...
        for human in humans:
            ...
            for pair_order, pair in enumerate(common.CocoPairsRender):
                ...
                cv2.line(wimg, centers[pair[0]], centers[pair[1]], common.CocoColors[pair_order], 3)
                cv2.imshow("3D", wimg)
